...
export const experienceService = {
  ...
  async getExperiences(
    ...
    if (params.occasions?.length)
      searchParams.set("occasions", params.occasions.join(","));
  ...
  async bulkDeleteExperiences(slugsOrIds: string[]): Promise<void> {
    await Promise.all(
      slugsOrIds.map((slugOrId) => this.deleteExperience(slugOrId))
    );
  },

  // Get experiences for frontend (only active)
  async getFrontendExperiences(
    params: {
      page?: number;
      limit?: number;
      search?: string;
  ...
  // Get similar experiences by category
  async getSimilarExperiences(
    ...
    searchParams.set("limit", (limit + 1).toString()); // Get one extra to filter out current
    searchParams.set("status", "active");
    searchParams.set("sortBy", "featured");
    ...
    // Filter out the current experience and limit results
...
export const getExperienceBySlug = (slug: string) =>
  experienceService.getExperienceBySlug(slug);
export const createExperience = (data: ExperienceForm) =>
  experienceService.createExperience(data);
export const updateExperience = (
  slugOrId: string,
  data: Partial<ExperienceForm>
) => experienceService.updateExperience(slugOrId, data);
export const deleteExperience = (slugOrId: string) =>
  experienceService.deleteExperience(slugOrId);
...
export const experienceUtils = {
  ...
  getActionButtonText(category: string): string {
    ...
    return giftCardCategories.includes(category)
      ? "Get Gift Card"
      : "Book Experience";
  },
